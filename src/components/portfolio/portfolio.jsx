import { React, useState } from "react";
import "./portfolio.css";
import Projects from "./project.js";

// import Me from '../../Assets/harsh9.png';

const Portfolio = ({ innerRef }) => {
  const content = [];
  let total = 365;
  for (let i = 0; i < total; i++) {
    content.push(i);
  }
  const list = [
    0, 1, 2, 3, 41, 42, 43, 44, 82, 83, 123, 124, 125, 126, 164, 165, 166, 167,
    205, 206, 246, 247, 287, 288, 5, 6, 46, 47, 87, 88, 128, 129, 169, 170, 210,
    211, 251, 252, 292, 293, 8, 9, 14, 15, 49, 50, 51, 90, 91, 92, 131, 132,
    133, 134, 172, 173, 213, 214, 254, 255, 295, 296, 175, 176, 217, 177, 218,
    259, 55, 56, 96, 97, 137, 138, 178, 179, 219, 220, 260, 261, 217, 218, 259,
    260, 261, 301, 302, 17, 18, 19, 58, 59, 60, 61, 99, 100, 102, 103, 140, 141,
    144, 181, 182, 185, 222, 223, 225, 226, 263, 264, 265, 266, 304, 305, 306,
    24, 25, 65, 66, 67, 106, 107, 108, 147, 148, 149, 150, 188, 189, 229, 230,
    270, 271, 311, 312, 67, 108, 149, 150, 191, 232, 192, 233, 274, 234, 275,
    316, 194, 235, 276, 154, 195, 236, 73, 114, 155, 33, 34, 74, 75, 115, 116,
    156, 157, 197, 198, 238, 239, 279, 280, 320, 321, 36, 37, 38, 39, 77, 78,
    79, 80, 118, 119, 159, 160, 161, 162, 200, 201, 202, 203, 241, 242, 282,
    283, 284, 285, 323, 324, 325, 326,
  ];

  const [year,setYear] = useState("first");

  return (
    <section id="portfolio" ref={innerRef}>
      <h5>My Reacent Work</h5>
      <h2>Portfolio</h2>

      <div className="projectSelect_Container">
         <div className="yearWise_project">
            <button className={year==="first"?"btn btn__active":"btn"} onClick={()=>setYear("first")}>First Year</button>
            <button className={year==="second"?"btn btn__active":"btn"} onClick={()=>setYear("second")}>Second Year</button>
            <button className={year==="third"?"btn btn__active":"btn"} disabled onClick={()=>setYear("third")}>Third Year</button>
            <button className={year==="forth"?"btn btn__active":"btn"} disabled onClick={()=>setYear("forth")}>Forth Year</button>
            <button className={year==="client"?"btn btn__active":"btn"} onClick={()=>setYear("client")}>Client Work</button>
         </div>
      </div>

      <div className="container portfolio__container">
        {Projects[year].map((e) => {
         console.log(e.year, year);
          return(
            <article className="portfolio__item" key={e.id}>
              <div className="portfolio__item-image">
                <img src={e.image} alt={e.alt} />
              </div>
              <h3>{e.name}</h3>
              <p>{e.description}</p>
              <div className="portfolio__item-cta">
                <a
                  href={e.gitUrl}
                  className="btn"
                  rel="noreferrer"
                  target="_blank"
                >
                  GitHub
                </a>
                <a
                  href={e.liveUrl}
                  className="btn btn-primary"
                  rel="noreferrer"
                  target="_blank"
                >
                  Live Demo
                </a>
              </div>
            </article>
          );
        })}
      </div>
      <div className="github__container">
        <div className="month_name">
          <span>Jan</span>
          <span>Feb</span>
          <span>Mar</span>
          <span>Apr</span>
          <span>May</span>
          <span>Jun</span>
          <span>Jul</span>
          <span>Aug</span>
          <span>Sep</span>
          <span>Oct</span>
          <span>Nov</span>
          <span>Dec</span>
        </div>
        <div className="github_box-container">
          {content.map((ind, i) => {
            return (
              <a
                href="https://github.com/mr-Harsh-Kumar-Jha"
                className={list.includes(i) ? "box active" : "box"}
              >
                <div key={ind}></div>
              </a>
            );
          })}
        </div>
        <a href="https://github.com/mr-Harsh-Kumar-Jha" className="githubTitle">
          ON GITHUB
        </a>
      </div>
    </section>
  );
};

export default Portfolio;
